...
async def run_champion_generation(
    ...
    TIMEOUT: If generation takes > 25s, returns the first candidate without judging.
    """
    import time
    ...
    # Renderer/vLLM-based judging has been removed from submitted solutions.
    # Deterministic fallback: pick the first texture candidate produced.
    final_ply_bytes = candidates[0].ply_file
    selected_seed = seed
    total_time = time.time() - generation_start
    logger.success(f"🏆 Selected candidate 1 (total time: {total_time:.2f}s)")
...
async def generate(
    ...
    """
    Generate PLY (Streaming). If seed = -1, service generates multiple candidates and selects deterministically (no renderer/judging).
    """
    try:
        logger.info(
...
async def generate_spz(
    ...
    """
    Generate SPZ (Compressed). If seed = -1, service generates multiple candidates and selects deterministically (no renderer/judging).
    """
    try:
        logger.info(
